var contactConfigs = {
    ...
    urls: {
        ...
        approve: apiUrl.contact.approve,
    ...
    btnApproveId: "#btnApprove",
    btnAddId: "#btnAdd",
    btnEditRow: "btnEditRow",
    laddaActiveBtn: Ladda.create(document.querySelector("#btnApprove")),
...
var contactFunctions = {
    ...
    initDatatable: function () {
        contactConfigs.$table = contactConfigs.$datatable.dataTable({
            ...
            "fnServerParams": function (aoData) {
                ...
                aoData.columns[8].search.value = contactConfigs.$customFilterByJobTitle.val();
                aoData.customParams = {
            ...
            "aoColumns": [
                ...
                },
                {
                    ...
                    "sClass": "text-center",
                    "render": function (data) {
                        if (contactConfigs.isInTag)
                            return `<a href="#" class="btn btn-xs btn-primary ${contactConfigs.btnEditRow}" data-id="${data.id}" ${tooltipHelper.edit(contactConfigs.tag)}><i class="fa fa-edit"></i></a>`;
                        return `<div><a href="${contactConfigs.urls.edit}/${data.id}" class="btn btn-xs btn-primary" ${tooltipHelper.edit(contactConfigs.tag)}><i class="fa fa-edit"></i></a></div>`;
                    }
                },
                {
                    "mData": "contactType",
                    ...
                    "render": function (value, type, data) {
                        if (data.supplierId) {
                            return `<a href="${contactConfigs.urls.editSupplier}/${data.supplierId}" class="btn btn-xs btn-primary pull-right" ${tooltipHelper.edit("Supplier")}><i class="fa fa-edit"></i></a> ${value} `;
                        } else if (data.customerId) {
                            return `<a href="${contactConfigs.urls.editCustomer}/${data.customerId}" class="btn btn-xs btn-primary pull-right" ${tooltipHelper.edit("Customer")}><i class="fa fa-edit"></i></a> ${value} `;
                        } else {
                            return value;
                        }
                    }
                ...
                },
                {
                    ...
                    "render": function (value, type, data) {
                        ...
                        return `<span class="label label-sm ${label}"> ${text} </span>`;;
                    }
                }
            ],
    ...
    active: function () {
        ...
        contactConfigs.laddaActiveBtn.start();
        App.blockUI({
            target: contactConfigs.tableId
        });
        $.ajax({
            url: contactConfigs.urls.approve,
            type: "POST",
            data: {
                "": contactConfigs.selected
            },
            success: function (result) {
                toastr["success"](`${contactConfigs.tag}(s) has been updated successful`);
            },
            error: function (result) {
                console.log(`error: ${result}`);
                toastr["error"](`${contactConfigs.tag}(s) failed to update, please try again`);
            },
            complete: function (result) {
                contactConfigs.selected = [];
                contactConfigs.laddaActiveBtn.stop();
    ...
    initEvents: function () {
        $(contactConfigs.btnApproveId).on("click", function () {
            contactFunctions.active();
        });
