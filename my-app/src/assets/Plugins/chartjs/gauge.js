...
(function() {
  ...
  Gauge = (function(superClass) {
    ...
    Gauge.prototype.renderStaticLabels = function(staticLabels, w, h, radius) {
      ...
      var maxWidth = this.ctx.measureText(formatNumber(9999.99 * 100, staticLabels.fractionDigits)).width;
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        ...
        if (value.label !== void 0) {
          ...
          if ((!this.options.limitMin || value >= this.minValue) && (!this.options.limitMax || value <= this.maxValue)) {
            ...
            var oldX = 0;
            var oldY = -radius - this.lineWidth * 0.5;

            var newX = oldX * Math.cos(rotationAngle) - oldY * Math.sin(rotationAngle);
            var newY = oldY * Math.cos(rotationAngle) + oldX * Math.sin(rotationAngle);

            var renderText = formatNumber(value, staticLabels.fractionDigits);

            var deltaX = maxWidth * Math.sin(rotationAngle) * 1;
            var subdeltaY = 5 * Math.cos(rotationAngle) * 1;
            var deltaY = ((subdeltaY + 5) / 2);

            this.ctx.translate(newX + deltaX, newY - deltaY);
            this.ctx.fillText(renderText, 0, 0);
            this.ctx.translate(-newX - deltaX, -newY + deltaY);
          }
        }
      }
